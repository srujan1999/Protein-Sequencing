...
def setupChartData(labels, proteinList):
    protein_list=combineProteins(proteinList)
    amino_dicts=aminoAcidDictionary(protein_list)
    frequency=[]
    for i in labels:
        if i in amino_dicts:
            frequency.append(amino_dicts[i]/len(protein_list))
        else:
            frequency.append(0)
    return frequency
...
def makeEdgeList(labels, biggestDiffs):
    diffs_acids=[i[0] for i in biggestDiffs]
    colour_labels=[]
    for i in labels:
        if i in diffs_acids:
            colour_labels.append("black")
        else:
            colour_labels.append("white")
